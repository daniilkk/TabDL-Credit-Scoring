...
def plot_feature_hist(df, feature):
    plt.hist(df[feature], bins=20, log=True)
...
def main(args: argparse.Namespace):
    ...
    # df.drop(df[(df['Age'] > 80) | (df['Age'] < 18)].index, inplace=True)
    # df.drop(df[(df['Num_Bank_Accounts'] > 20)].index, inplace=True)
    # df.drop(df[(df['Annual_Income'] > 1e10)].index, inplace=True)
    # df.drop(df[(df['Num_Credit_Card'] > 11)].index, inplace=True)
    # df.drop(df[(df['Interest_Rate'] > 34)].index, inplace=True)
    # df.drop(df[(df['Changed_Credit_Limit'] > 10000)].index, inplace=True)
    # df.drop(df[(df['Num_of_Delayed_Payment'] > 28)].index, inplace=True)
    # df.drop(df[(df['Num_Credit_Inquiries'] > 170)].index, inplace=True)
